...
class ParsingTables {
    ...
    /** Default action for each state */protected
    ...
    protected void insertActions(Action.List action_list, int offset)
     {
    ...
    private boolean canInsertActions(Action.List action_list, int offset)
     {
        ...
        return true;
    } void insertLookaheads(Action.List action_list, int offset)
     {
        for (Action act = action_list.first; act != null; act = act.next)
         {
            int index = offset + act.lookahead.id;
