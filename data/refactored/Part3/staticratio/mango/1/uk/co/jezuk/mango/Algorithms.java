...
public class Algorithms {
   ...
   */
    private
     static <T> void remove(Collection<? extends  T> collection, T value) { Remove.execute(collection.iterator(), value); }

    private
     static <T> void remove(Collection<? extends  T> collection, int start, int end, T value) { Remove.execute(Iterators.BoundedIterator(collection.iterator(), start, end), value); }

    private
     static <T> void remove(List<? extends  T> list, int start, int end, T value) { Remove.execute(Iterators.BoundedIterator(list, start, end), value); }

    private
     static <T> void remove(Iterator<? extends  T> iterator, T value) { Remove.execute(iterator, value); }
 ...
 /**
   ...
   */ public <T> List<T> partition(Collection<T> collection,
                                      Predicate<? super  T> pred)
     { return Partition.execute(collection.iterator(), pred, new ArrayList<T>()); } public <T, C extends  Collection<? super  T>>
                      C partition(Collection<T> collection,
                                  Predicate<? super  T> pred,
                                  C results)
     { return Partition.execute(collection.iterator(), pred, results); } public <T> List<T> partition(Collection<T> collection,
                                       int start,
                                       int end,
                                       Predicate<? super  T> pred)
     { return Partition.execute(Iterators.BoundedIterator(collection.iterator(), start, end), pred, new ArrayList<T>()); } public <T, C extends  Collection<? super  T>>
                      C partition(Collection<T> collection,
                                  int start,
                                  int end,
                                  Predicate<? super  T> pred,
                                  C results)
     { return Partition.execute(Iterators.BoundedIterator(collection.iterator(), start, end), pred, results); } public <T> List<T> partition(List<T> list,
                                      int start,
                                      int end,
                                      Predicate<? super  T> pred)
     { return Partition.execute(Iterators.BoundedIterator(list, start, end), pred, new ArrayList<T>()); } public <T, C extends  Collection<? super  T>>
                      C partition(List<T> list,
                                  int start,
                                  int end,
                                  Predicate<? super  T> pred,
                                  C results)
     { return Partition.execute(Iterators.BoundedIterator(list, start, end), pred, results); } public <T> List<T> partition(Iterator<T> iterator,
                                      Predicate<? super  T> pred)
     { return Partition.execute(iterator, pred, new ArrayList<T>()); } public <T, C extends  Collection<? super  T>>
   ...
   * @see Iterator
   * @see Comparator
   */ public <T> void unique(Collection<T> collection) { Unique.execute(collection.iterator(), null); } public <T> void unique(Collection<T> collection, int start, int end) { Unique.execute(Iterators.BoundedIterator(collection.iterator(), start, end), null); } public <T> void unique(List<T> list, int start, int end) { Unique.execute(Iterators.BoundedIterator(list, start, end), null); } public <T> void unique(Iterator<T> iterator) { Unique.execute(iterator, null);   } public <T> void unique(Collection<T> collection, Comparator<? super  T> comparator) { Unique.execute(collection.iterator(), comparator);  } public <T> void unique(Collection<T> collection, int start, int end, Comparator<? super  T> comparator) { Unique.execute(Iterators.BoundedIterator(collection.iterator(), start, end), comparator);   } public <T> void unique(List<T> list, int start, int end, Comparator<? super  T> comparator) { Unique.execute(Iterators.BoundedIterator(list, start, end), comparator); } public <T> void unique(Iterator<T> iterator, Comparator<? super  T> comparator) { Unique.execute(iterator, comparator); }
