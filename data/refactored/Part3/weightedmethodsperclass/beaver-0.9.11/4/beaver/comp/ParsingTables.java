...
class ParsingTables {
    ...
    /** Actions lookup "table" to be used with terminal lookahead symbols */public
     short[] actions;
    ...
    private static void renumberSymbols(Grammar grammar, Action.List[] action_lists)
     {
        for (int i = 0; i < action_lists.length; i++)
         {
    ...
    private int advanceStartIndex(int start_index)
     {
        while (start_index < actions.length && actions[start_index] != 0)
            start_index++;
        return start_index;
    } int findOffset(Action.List action_list, int start_index)
     {
