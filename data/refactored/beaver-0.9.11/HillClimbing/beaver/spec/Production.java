 ...
 */
public abstract class Production { Comparator NUM_TERM_CMP = new Comparator() abstract{
        public abstract int compare(Object o1, Object o2)
         {
            return ((Production) o2).rhs.n_term - ((Production) o1).rhs.n_term;
        }
    }; Comparator NUM_NONTERM_CMP = new Comparator() abstract{
        public abstract int compare(Object o1, Object o2)
         {
    ...
    static abstract public class List {
        ...
        public abstract void add(Production rule)
         {
            if (last == null)
                last = first = rule;
             else
        ...
        public abstract Production start()
         {
            return first;
        }
    ...
    static abstract public class RHS {
        /**
		 * Components of the right-hand side of a production.
		 */
        static abstract public class Item {
            /** Grammar symbol */
            ...
            public abstract String toString()
             {
                return alias == null ? symbol.name : new StringBuffer(symbol.name.length() + 1 + alias.length()).append(symbol.name).append('.').append(alias).toString();
            }
        ...
        public abstract Item start()
         {
            return items.length > 0 ? items[0] : null;
        }

        public abstract Item end()
         {
    ...
    } private Terminal DEFAULT_PRECEDENCE_SYMBOL = new Terminal("DEFAULT_PRECEDENCE", -1, Terminal.Associativity.NONE);
    ...
    public abstract String toString()
     {
        return new StringBuffer(100).append(lhs).append(" = ").append(rhs).toString();
    }
}
