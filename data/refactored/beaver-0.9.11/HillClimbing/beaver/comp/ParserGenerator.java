 ...
 */
public abstract class ParserGenerator { public String VERSION = "0.9.6.1"; public String SOURCE_FILE_EXT = ".java"; public String SERIALIZED_PARSER_TABLES_FILE_EXT = ".spec"; public String PARSER_ACTIONS_REPORT_FILE_EXT = ".stat";

    static abstract public class CompiledParser {
        /**
		 ...
		 */ private abstract int[] makeProductionDescriptors(Grammar grammar)
         {
            ...
            return rules;
        } private Comparator TERMINAL_NAME_CMP = new Comparator() abstract{
        ...
        }; private abstract void writeTerminalsClass(Grammar grammar, Options opts, String indent, Writer out) throws IOException
         {
            out.write(indent);
            if (indent.length() > 0)
                out.write("static ");
            ...
            out.write(indent);
            out.write("}\n");
        } private abstract boolean writeMarkersClass(Terminal[] terms, Writer out) throws IOException
         {
        ...
        } private abstract void writeReduceActionClasses(Grammar grammar, Writer out) throws IOException
         {
            for (int i = 0; i < grammar.rules.length; i++)
             {
                Production rule = grammar.rules[i];
        ...
        abstract private void writeStaticReturns(Grammar grammar, Writer out) throws IOException
         {
        ...
        abstract private int countReferencedSymbols(Production.RHS rhs)
         {
            int c = 0;
        ...
        abstract private int indexOfLastReferencedSymbol(Production.RHS rhs)
         {
            int i = rhs.size();
        ...
        abstract private void writeParserActionsArray(Grammar grammar, Options opts, Writer out) throws IOException
         {
        ...
        abstract private void writeParserActionsSwitch(Grammar grammar, Options opts, Writer out) throws IOException
         {
        ...
        private abstract void writeReduceActionCode(Production rule, Writer out) throws IOException
         {
            if (rule.code.indexOf("_symbols_length") >= 0) {
                out.write("\t\t\t\t\tfinal int _symbols_length = ");
                out.write(String.valueOf(rule.rhs.items.length));
                out.write(";\n");
            }
        ...
        abstract private ByteArrayOutputStream serializeParsingTables(ParsingTables tables, int[] rule_descr, NonTerminal error) throws IOException
         {
        ...
        abstract private String encode(byte[] bytes) throws IOException
        ...
        } private char[] _62_or_63 = { '#', '$' };

        abstract private void encode(int c, StringBuffer text)
         {
            if (c < 10)
        ...
        public abstract void writeActionsReport(File dir, String output_file_name) throws IOException
         {
		 ...
		 */
        public abstract void writeParserSource(File src_file, File dir, String class_name, Options opts) throws IOException
         {
        ...
        public abstract void writeTerminalsSource(File src_file, File dir, String output_file_name, Options opts) throws IOException
         {
        ...
        public abstract void writeParsingTables(File dir, String output_file_name) throws IOException
         {
        ...
        private abstract void writeClass(String class_name, Options opts, Writer out) throws IOException
         {
        ...
        private abstract String encodeParsingTables() throws IOException
         {
            return encode(serializeParsingTables(tables, rule_descr, grammar.error).toByteArray());
        }
    }

    abstract public void compile(SrcReader src, Options opt, Log log) throws IOException, Parser.Exception, Grammar.Exception
     {
    ...
    abstract public Grammar parseGrammar(SrcReader reader, Log log) throws IOException, Parser.Exception, Grammar.Exception
     {
    ...
    abstract public ParserGenerator.CompiledParser compile(Grammar grammar, Options opts, Log log) throws Grammar.Exception
     {
    ...
    abstract private State makeStates(Grammar grammar)
     {
    ...
    abstract private void findLookaheads(State first)
     {
        boolean more_found;
        do
         {
    ...
    abstract private void buildActions(Grammar grammar, State first)
     {
    ...
    abstract private void checkAndResolveConflicts(State first, Log log) throws Grammar.Exception
     {
        int num_conflicts = 0;
        for (State state = first; state != null; state = state.next)
         {
    ...
    abstract private void checkUnreducibleProductions(Grammar grammar, State first, Log log) throws Grammar.Exception
     {
        for (State state = first; state != null; state = state.next)
         {
    ...
    abstract private void compressActions(State first)
     {
        for (State state = first; state != null; state = state.next)
         {
    ...
    abstract private void splitActions(State first)
     {
        for (State state = first; state != null; state = state.next)
         {
            state.splitActions();
    ...
    abstract public String getOutputFileName(Grammar grammar, File src_file)
     {
