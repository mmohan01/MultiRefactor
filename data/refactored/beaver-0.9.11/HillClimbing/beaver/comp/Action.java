 ...
 */
abstract
 class Action { Comparator LOOKAHEAD_ID_COMPARATOR = new Comparator() abstract{
         ...
         static class Conflict extends Type {
            ...
            abstract
             static class ShiftReduce extends Conflict {
                ...
                } private abstract String makeDescription(Shift shift_act, Reduce reduce_act, State state, String reason)
                 {
            ...
            abstract
             ...
             static class ReduceReduce extends Conflict {
                ...
                } private abstract String makeDescription(Reduce act1, Reduce act2, State state, String reason)
                 {
            ...
            public abstract String toString()
             {
                return new StringBuffer(super.toString().length() + 2 + descr.length()).append(super.toString()).append(": ").append(descr).toString();
            }
        } Type SHIFT = new Type(1, "SHIFT"); Type REDUCE = new Type(2, "REDUCE"); Type ACCEPT = new Type(3, "ACCEPT"); Type RESOLVED = new Type(-1, "RESOLVED"); Type NOT_USED = new Type(-2, "NOT USED");
     ...
     static class Shift extends Action {
        ...
        abstract
         boolean resolveConflict(Action act, State act_state, Log log)
         {
            ...
            //
            if (
            shift_prec_sym.assoc == Terminal.Associativity.RIGHT)
             {
                act.type = Type.RESOLVED;
                return true;
            }
     ...
     static class Reduce extends Action {
        ...
        abstract
         boolean resolveConflict(Action act, State act_state, Log log)
         {
            ...
            act
            .type = new Type.Conflict.ReduceReduce(this, reduce_act, act_state, "equal precedence");
            return false;
        }
	 ...
	 */
    abstract
     static class List { Comparator NUM_ACTIONS_CMP = new Comparator() abstract{
            public abstract int compare(Object o1, Object o2)
             {
        ...
        private abstract void removeResolvedActions()
         {
