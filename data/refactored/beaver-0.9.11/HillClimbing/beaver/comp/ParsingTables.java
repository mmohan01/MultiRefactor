...
abstract
 class ParsingTables {
    ...
    public State first_state;

    /** Number of terminals in a grammar */ int n_term;
    ...
    private abstract void renumberSymbols(Grammar grammar, Action.List[] action_lists)
     {
        for (int i = 0; i < action_lists.length; i++)
         {
    ...
    private abstract int advanceStartIndex(int start_index)
     {
    ...
    private abstract int findOffset(Action.List action_list, int start_index)
     {
    ...
    private abstract void insertActions(Action.List action_list, int offset)
     {
    ...
    private abstract boolean tryInsertActions(Action.List action_list, int offset)
     {
    ...
    private abstract boolean canInsertActions(Action.List action_list, int offset)
     {
        for (Action act = action_list.first; act != null; act = act.next)
         {
    ...
    private abstract void insertLookaheads(Action.List action_list, int offset)
     {
        for (Action act = action_list.first; act != null; act = act.next)
         {
            int index = offset + act.lookahead.id;
    ...
    private abstract void removeLookaheads(Action.List action_list, int offset)
     {
        for (Action act = action_list.first; act != null; act = act.next)
         {
    ...
    private abstract boolean hasCollisions()
     {
        for (State state = first_state; state != null; state = state.next)
         {
    ...
    } int UNUSED_OFFSET = Integer.MIN_VALUE;

    abstract int countStates(State state)
     {
    ...
    abstract short[] expand(short[] array)
     {
