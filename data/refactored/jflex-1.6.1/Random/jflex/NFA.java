...
final public class NFA {
  ...
  */
        ...
        for (int i = 0; i < numStates; i++) {
            ...
            for (int input = 0; input < numInput; input++) {
                if ( table[i][input] != null && table[i][input].containsElements())
                 result.append("  with ").append((int) input).append(" in ").append(table[i][input]).append(Out.NL);
            }
  ...
  // Functions for constructing NFAs out of regular expressions.
    ...
    } IntPair insertStringNFA(boolean caseless, String str) {
        int start = numStates;
        int i = 0;
