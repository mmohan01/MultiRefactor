 ...
 * @author AL
 */
public abstract class PrependExpressionWithStatements extends TwoPassTransformation {
    ...
    public ProblemReport analyze() {
        ...
        if (report == IDENTITY) {
            ...
            if (i >= j) {
                for (--j, --i; j >= 0; --j, --i) {
                    if (!ProgramElement.STRUCTURAL_EQUALITY.equals(statements.get(j), grandpa.getStatementAt(i))) {
                        break;
                    }
                }
                if (j < 0) {
