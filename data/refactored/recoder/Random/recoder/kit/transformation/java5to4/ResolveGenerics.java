...
public class ResolveGenerics extends TwoPassTransformation {
     ...
     public ProblemReport analyze() {
        ...
        while (fw.next()) {
            ...
            if (pe instanceof VariableReference) {
                ...
                ASTList<
                TypeArgumentDeclaration> typeArgs =
                    pe instanceof MethodReference ? ((MethodReference)pe).getTypeArguments() : null;
                removeTypeArgs(typeArgs);
                // add cast if required.
                Method m =
                    pe instanceof MethodReference ? si.getMethod((MethodReference)pe) : si.getConstructor((ConstructorReference)pe);
                ...
                int i =
                    mr instanceof MethodReference ?
                            si.getMethods(m.getContainingClassType(), m.getName(), transSig, MiscKit.getParentTypeDeclaration(mr)).size() :   si.getConstructors(m.getContainingClassType(), transSig, MiscKit.getParentTypeDeclaration(mr)).size();
                if (i > 1) {
    ...
    private void handleTypeParameterInThrows(TypeReference pe) {
        ...
        for (int i = 0; i < mrs.size(); i++) {
            ...
            if (methodReferenceResolved instanceof ResolvedGenericMethod) {
                ...
                List<
                ClassType> declaredExceptions = Collections.emptyList();
                NonTerminalProgramElement npe = mr;
                for (;;) {
                    npe = npe.getASTParent();
                    if (npe instanceof Try) {
                        break;
                    } // must handle *all* exceptions (I think??)
                    if (
                    npe instanceof MethodDeclaration) {
                    ...
                    if (npe == null) {
                        System.out.println("???");
                        break;
                    } // hopefully we only got runtime exceptions, otherwise it would be weird.
                }
                ...
                ASTList<
                ParameterDeclaration> pds = new ASTArrayList<ParameterDeclaration>(md.getParameterDeclarationCount() + 1);
                ...
                Statement stmnt = f.createMethodReference(
                                md.isStatic() ? TypeKit.createTypeReference(crs, md.getMemberParent(), mr) : f.createVariableReference(f.createIdentifier("___callTgt")),
    ...
    private boolean checkCastRequired(Reference mr, Type actual, Type required) {
        ...
        if (
        required == getNameInfo().getJavaLangObject() || required == getNameInfo().getJavaLangObject().getErasedType())
    ...
    @Override
     public void transform() {
        super.transform();
        System.out.println("Remove: " + toRemove.size() + " repl: " + referenceReplacements.size() + " casts: " + casts.size() + " prophylaxis performed: " + castAllParams.size() + " guard methods inserted: " + guardMethods.size());
        ...
        for (
        int i = referenceReplacements.size() - 1; i >= 0; i--) {
