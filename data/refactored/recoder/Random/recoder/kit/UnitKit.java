...
public class UnitKit {
    ...
    private static ClassType getNecessaryImportedType(CrossReferenceSourceInfo xi, Import imp) {
        ...
        if (
        TypeKit.getReferences(xi, ct, imp.getASTParent(), false).size() > 1) {
            return ct;
        } else {
            return null;
        }
    ...
    public static void normalizeImports(ChangeHistory ch, CrossReferenceSourceInfo xrsi, CompilationUnit cu,
        ...
        Set<
        ClassType> importTypes = new HashSet<ClassType>();
        ...
        for (
        int i = cu.getTypeDeclarationCount() - 1; i >= 0; i -= 1) {
        ...
        for (int i = ilsize - 1; i >= 0; i -= 1) {
            ...
            if (
            (imp.isMultiImport() && removeMultiTypeImports)
    ...
    public static void sortImports(ChangeHistory ch, CompilationUnit cu) {
        ...
        for (int i = 0; i < il.size(); i += 1) {
            ...
            if (imp.isStaticImport() && !imp.isMultiImport()) {
                ...
                names[
                i] = Naming.toPathName(ref.getReferencePrefix(), "." + ref.getName());
            } else {
