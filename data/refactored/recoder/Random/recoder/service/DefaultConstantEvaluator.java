...
public class DefaultConstantEvaluator extends AbstractService implements ConstantEvaluator {
            ...
            case BYTE_TYPE:
                switch (rhs.getTypeCode()) {
                    case SHORT_TYPE:
                        ...
                        return; // byte x short -> short x short
                    case CHAR_TYPE:
                        promoteNumericTypeToInt(lhs);
                        promoteNumericTypeToInt(rhs);
                        return; // byte x char -> int x int
                }
                break;
            case CHAR_TYPE:
                switch (rhs.getTypeCode()) {
            ...
            case SHORT_TYPE:
                switch (rhs.getTypeCode()) {
                    case BYTE_TYPE:
                        ...
                        return; // short x byte -> short x short
                    case CHAR_TYPE:
                        promoteNumericTypeToInt(lhs);
                        promoteNumericTypeToInt(rhs);
                        return; // short x char -> int x int
                }
                break;
        }
        ...
        if (expr instanceof Operator) {
            ...
            switch (op.getArity()) {
                case 1: // unary operations
                    ...
                    if (
                    !isCompileTimeConstant(op.getExpressionAt(0), res)) {
                        return false;
                    }
                ...
                // this must be the conditional (?:)
                    ...
                    return true;
                // evaluate both sides; finish if not constant // overwrite old values
            }
    ...
    final static BinaryBooleanOperation LESS_THAN = new BinaryBooleanOperation() private{
        public boolean eval(boolean a, boolean b) {
            fail();
            return false;
        }
