        ...
        if (ctc instanceof ParameterizedType) {
            ...
            for (ClassType ct: genericTypes) {
                if (ct.isInner())
                    // TODO check this
                    res.
                add(getParameterizedType(ct, null, (ParameterizedType)ctc));
                 else
                    res.add(ct);
            }
            return res;
    ...
    private class WrappedParameterizedType {
        ...
        @Override
         public boolean equals(Object o) {
            ...
            if (
            pt.getGenericType() != opt.getGenericType())
                return false;
            ...
            for (int i = 0; i < ta1.size(); i++) {
                ...
                if (
                _1 instanceof CapturedTypeArgument && ((CapturedTypeArgument)_1).getTypeArgument() instanceof TypeArgumentDeclaration)
                    return false; // Can't reuse.
                if (
                _2 instanceof CapturedTypeArgument && ((CapturedTypeArgument)_2).getTypeArgument() instanceof TypeArgumentDeclaration)
                    return false; // Can't reuse.
                if (
                !ta1.get(i).semanticalEquality(ta2.get(i))) {
                    return false;
                }
            }
            return true;
