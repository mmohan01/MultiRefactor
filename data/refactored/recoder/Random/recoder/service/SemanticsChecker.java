    ...
    private class SemanticsCheckerVisitor extends SourceVisitor {
        ...
        @Override
         public void visitEnhancedFor(EnhancedFor ef) {
            ...
            if (rhsType instanceof ArrayType) {
                ...
                List<
                ClassType> iterables = new ArrayList<ClassType>();
                ...
                if (iterables.size() == 0) {
                    ...
                    if (
                    lhsType instanceof PrimitiveType)
        ...
        private void checkCompatible(Type lhs, Expression rhsExpr) {
            ...
            if (!si.isWidening(
                ...
                errorHandler.
                reportError(new TypingException("Invalid assignment", rhsExpr));
            }
        }
        ...
        private final boolean occursInStaticContext(FieldReference fr) {
            ...
            System
            .out.println(UnitKit.getCompilationUnit(fr) + " " + fr.getStartPosition());
        ...
        private void checkRawTypeOk(TypeReference tr) {
            if (tr.getReferencePrefix() != null && tr.getReferencePrefix() instanceof TypeReference) {
                ...
                if (
                !(trType instanceof ParameterizedType) &&
        ...
        @Override
         public void visitSwitch(Switch x) {
            ...
            if (
            java5allowed && si.isWidening(exprType, ni.getJavaLangEnum()))
                return; // ok - enum.
            errorHandler.
            reportError(new TypingException("Invalid type in Switch-expression: " + exprType.getFullName(), x.getExpression()));
        }
        ...
        @Override
         public void visitCase(Case x) {
            ...
            if (constType instanceof PrimitiveType) {
                ...
                if (
                !fitsIntoForSwitch(x.getExpression(), (PrimitiveType)switchExprType)) {
                    fitsIntoForSwitch(x.getExpression(), (PrimitiveType)switchExprType); // debug.
                    System
                    .out.println(x.getExpression().toSource());
            ...
            if (
            ((ClassType)constType).isEnumType() && ((FieldReference)x.getExpression()).getReferencePrefix() != null)
