...
public class DefaultSourceInfo extends DefaultProgramModelInfo implements SourceInfo, ChangeHistoryListener, Formats {
    ...
    void processChange(TreeChange change) {
        ...
        if (
        changed instanceof PackageSpecification) {
        ...
        if (npe instanceof TypeDeclaration) {
            ...
            tc instanceof AttachChange) {
                ...
                if (old == td) // special handling which might occur if a nameclash happened before, which would now be resolved
                    dni.
                unregisterClassType(td.getFullName());
            }
    ...
    private ClassType getFromTypeImports(String name, List<Import> il) {
        ...
        for (int i = il.size() - 1; i >= 0; i -= 1) {
            ...
            if (
            newResult != null && (newResult.isStatic() || !imp.isStaticImport())) {
                if (result != null && result != newResult) {
                    getErrorHandler().reportError(
                            new AmbiguousImportException("Ambiguous import " + Format.toString(ELEMENT_LONG, imp) + " - could be " + Format.toString("%N", result) + " or " + Format.toString("%N", newResult), imp, result, newResult));
        ...
        for (int i = il.size() - 1; i >= 0; i--) {
            ...
            if (imp.isMultiImport()) {
                ...
                newResult != null && (newResult.isStatic() || !imp.isStaticImport())) {
                    if (result != null && result != newResult) {
                        getErrorHandler().reportError(
                                new AmbiguousImportException("Ambiguous import of type " + name + ": could be " + Format.toString("%N", result) + " or " + Format.toString("%N", newResult),
    ...
    private ClassType getInheritedType(String name, ClassType ct) {
        ...
        List<
        ClassType> ctl = getAllTypes(ct);
        if (DEBUG)
            Debug.log("Checking type " + shortName + " as inherited member of " + ct.getFullName() + ": " + Format.toString("%N", ctl));
        ClassType result = null;
        int nc = ctl.size();
    ...
    public Type getType(String name, ProgramElement context) {
        ...
        name.endsWith("]")) {
            ...
            // compute base type
            t
             = getType(name.substring(0, px), context);
            if (t == null) {
                return null;
            }
...
//        }
        ...
        if (
        isPartOf(context, TypeParameterDeclaration.class)) {
        ...
        if (scope == null) {
            Debug.log("Null scope during type query " + name + "(" + context.getID() + ") in context " + Format.toString(Formats.ELEMENT_LONG, context));
        ...
        if (result == null) {
            ...
            if (result == null && il != null) {
                ...
                result = getFromPackageImports(name, il,
                        cu.getTypeDeclarationCount() == 0 ? null : cu.getTypeDeclarationAt(0 /* doesn't matter which one to check, since this is important for static imports only */));
            }
        }
        if (result == null) {
    ...
    public Type getType(VariableSpecification vs) {
        ...
        if (
        vs.getParent() instanceof UnionTypeParameterDeclaration) {
        ...
        if (expr instanceof Operator) { ///////////////// Operators
            ...
            if (op instanceof Assignment) {
                ...
                if (op instanceof Plus &&
                    ...
                    if ((op instanceof Plus)
                        ...
                        if ((t1 instanceof PrimitiveType) && (t2 instanceof PrimitiveType)) {
                            ...
                            if ((t1 != null) && (t2 != null)) {
                                getErrorHandler().reportError(
                                        new TypingException("Illegal operand types for plus " + t1 + " + " + t2 + " in expression " + op, op));
                                result = getNameInfo().getUnknownType();
                            }
                        }
                    }
                ...
                if (t1 instanceof PrimitiveType && t2 instanceof PrimitiveType) {
                    ...
                    if (t1 == getNameInfo().getNullType()) {
                        ...
                        if (
                        t1 instanceof ErasedType || t2 instanceof ErasedType) {
            ...
            if (expr instanceof UncollatedReferenceQualifier) {
                ...
                if (java5Allowed()) {
                    ...
                    if (
                    pref_type instanceof PrimitiveType)
                    ...
                    ArrayList<
                    TypeArgument> tas = new ArrayList<TypeArgument>(1);
                    tas.add(ta);
                ...
                if (m != null) {
                    ...
                    if (result instanceof ClassType && ((ClassType)result).isInner()) {
                        ...
                        if (mr.getReferencePrefix() != null) {
                            ...
                            if (
                            pref instanceof ParameterizedType) {
                ...
                if (rp == null) {
                    ...
                    ((TypeDeclaration)pe).getName() == null || // pe.getName() == null in case of anonymous classes.
                            (
        ...
        for (int j = 0; j < sig.size(); j++) {
            ...
            m.isVarArgMethod() && j == sig.size() - 1) {
                ...
                if (mrArgsSz == sig.size()) {
                    ...
                    if (
                    actualTypeArg instanceof ArrayType)
                    ...
                    for (int k = sig.size() - 1; k < mrArgsSz; k++) {
                        ...
                        inferType1(
                        typeParamName, result, t, actualArgType.createArrayType());
                    }
                }
            } else {
                ...
                if (actualArgType == getNameInfo().getNullType())
                    continue; // Don't infer from null type.
                inferType1(
                typeParamName, result, t, actualArgType);
            }
        }
        if (result.size() == 0) {
    ...
    private boolean inferFromContext(String typeParamName, List<ClassType> result, Method m, Type expectedFromContext) {
        ...
        for (
        ClassType t: ret.getAllSupertypes()) {
    ...
    private void inferType1(String typeParamName, List<ClassType> result, Type t, Type actualArgType) {
        ...
        if (t instanceof ParameterizedType && actualArgType instanceof ClassType) {
            ...
            if (inferFrom == null)
                return; // skip
            for (
            int i = 0; i < tp.getAllTypeArgs().size(); i++) {
...
//        	EnumConstant ec = (EnumConstant)((ClassType)getType(((Case)context.getASTParent()).getParent().getExpression())).getVariableInScope(name)
        ...
        if (result == null && java5Allowed()) {
            ...
            if (result == null) // try on demand
                result
             = getVariableFromStaticOnDemandImport(name, imports, ct);
        }
        return result;
    }
    ...
    public Variable getVariableFromStaticSingleImport(String name, List<Import> imports, ClassType context) {
        ...
        int i = 0, max = imports.size(); i < max; i++) {
            ...
            for (
            int f = 0, maxF = fields.size(); f < maxF; f++) {
                ...
                // also see inferType() !
                if (
                context == null || isVisibleFor(field, context)) {
                    result = field;
                    ...
                    oldResult = field;
                    break;
                } // maximum of one match per import
            }
        }
        return result;
    }
...
//            }
    ...
    public List<Type> makeSignature(List<Expression> args) {
        ...
        for (int i = 0; i < arity; i++) {
            ...
            if (et == null) {
                getErrorHandler().reportError(
                        new TypingException("Unknown type for argument #" + i + " in call " + Format.toString(ELEMENT_LONG, e.getExpressionContainer()), e));
    ...
    public List<Method> getMethodsFromStaticOnDemandImports(MethodReference mr, List<Import> imports) {
        ...
        for (int i = 0, max = imports.size(); i < max; i++) {
            ...
            for (int j = 0; j < ml.size(); j++) {
                ...
                if (
                m.isStatic() && m.getName() == mr.getName())
                    result.add(m);
            }
        }
    ...
    public List<Method> getMethodsFromStaticSingleImports(MethodReference mr, List<Import> imports) {
        ...
        for (int i = 0, max = imports.size(); i < max; i++) {
            ...
            if (
            imp.getStaticIdentifier().getText() != mr.getName())
                continue;
            ClassType ct = ni.getClassType(Naming.toPathName(imp.getTypeReference()));
            if (ct == null) // unknown type. Already reported, and the unresolved method will be reported as well.
                return Collections.emptyList();
    ...
    public List<? extends  Constructor> getConstructors(ConstructorReference cr) {
        ...
        if (cr instanceof New) {
            ...
            for (int i = 0; i < superTypes.size(); i += 1) {
                ...
                if (!type.isInterface()) {
                    break;
                } // there must be one concrete class
    ...
    List<ClassType> getTypeListFromTypeArgs(List<TypeArgumentDeclaration> targs) {
        ...
        for (TypeArgumentDeclaration tad: targs) {
            ...
            res.
            add((ClassType) getType(tad.getTypeReference()));
        }
        return res;
    }
                ...
                if (con instanceof New) {
                    ...
                    if (ext == null || ext.getSupertypes().size() == 0) {
                        if (cd != jlo) // implicitly
                            res.
                        add(getNameInfo().getJavaLangObject());
                    } else {
                        addToTypeList(res, ext.getSupertypes());
                    }
    ...
    public List<Type> getSignature(Method m) {
        ...
        if (md == null) {
            ...
            List<
            ParameterDeclaration> pdl = md.getParameters();
    ...
    private Reference resolveURQ(UncollatedReferenceQualifier urq, boolean allowVariables) {
        ...
        if (result == null) {
            getErrorHandler().reportError(
                    new UnresolvedReferenceException(Format.toString("Could not resolve " + ELEMENT_LONG + " (11)", urq), urq));
            result = urq;
        } else if (result != urq) {
            try {
                    ...
                    if (i > 0) {
                        ...
                        for (int j = i - 1; j >= 0; j -= 1) {
                            if (branches.get(j) instanceof Catch) {
                                ClassType dx = (ClassType) getType(((Catch) branches.get(j)).getParameterDeclaration().getTypeReference());
                                if (isSubtype(ex, dx)) {
    ...
    private void analyzeProgramElement0(ProgramElement pe) {
        ...
        while (!worklist.isEmpty()) {
            ...
            if (
            pe instanceof ScopeDefiningElement) {
