...
public class DefaultByteCodeInfo extends DefaultProgramModelInfo implements ByteCodeInfo {
    ...
    public List<ClassType> getSupertypes(ClassType ct) {
        ...
        // TODO cache / register (?)
        if (
        ct instanceof TypeParameterInfo) {
        ...
        Debug.
        assertNonnull(cfce.supertypes); // created during registration
    ...
    public void register(ClassFile cf) {
        ...
        if (innerClasses != null) {
            ...
            for (int i = 0; i < innerClasses.length; i++) {
                ...
                if (cn.equals(fullName)) {
                    ...
                    // STUPID
                ni.
                getClassType(cn); // bad, bad side-effect programming ;)
                /*
        ...
        if (sname != null) {
            ...
            if (ct == null) {
                getErrorHandler().reportError(
                        new MissingClassFileException("Unknown byte code supertype " + sname + " in class " + cf.getFullName(), sname));
            } else {
                List<TypeArgumentInfo> tais = cf.getSuperClassTypeArguments();
                // TODO 0.90
                if (
                tais != null && tais.size() > 0)
        ...
        for (int i = 0; i < inames.length; i++) {
            ...
            if (ct == null) {
                getErrorHandler().reportError(
                        new MissingClassFileException("Unknown byte code supertype " + iname + " in class " + cf.getFullName(), iname));
            } else {
