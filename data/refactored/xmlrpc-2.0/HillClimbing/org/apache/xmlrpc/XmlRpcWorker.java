 ...
 * @since 1.2
 */
public abstract class XmlRpcWorker {
     ...
     */
    protected abstract Object invokeHandler(Object handler, XmlRpcServerRequest request, XmlRpcContext context)
        throws Exception
     {
        long now = 0;
        ...
        try
         {
            ...
            if (handler == null)
             ...
             {
                return ((ContextXmlRpcHandler) handler).execute(request.getMethodName(), request.getParameters(), context);
            }
             else if (handler instanceof XmlRpcHandler)
             {
                return ((XmlRpcHandler) handler).execute(request.getMethodName(), request.getParameters());
            }
             else if (handler instanceof AuthenticatedXmlRpcHandler)
             {
                return ((AuthenticatedXmlRpcHandler) handler).execute(request.getMethodName(), request.getParameters(),
     ...
     */
    public abstract byte[] execute(InputStream is, String user, String password)
     {
     ...
     * supplied.
     */
    public abstract byte[] execute(InputStream is, XmlRpcContext context)
     {
        long now = 0;
     ...
     */
    protected abstract XmlRpcContext defaultContext(String user, String password)
     {
