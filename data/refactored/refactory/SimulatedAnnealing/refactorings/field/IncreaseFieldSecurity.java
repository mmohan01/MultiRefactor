...
public abstract class IncreaseFieldSecurity extends Refactoring {
    ...
    public abstract ProblemReport analyze(int iteration, int unit, int element)
     {
        ...
        super.refactoringInfo = "Iteration " + iteration + ": \"Increase Field Security\" applied at class " + super.getFileName(getSourceFileRepository().getKnownCompilationUnits().get(unit).getName()) + " to element " + pe.getClass().getSimpleName() + " (" + pe.toString().substring(last + 2) + ") from " + super.currentModifier(vd.getVisibilityModifier()) + " to " + super.refactoredUpModifier(vd.getVisibilityModifier());
