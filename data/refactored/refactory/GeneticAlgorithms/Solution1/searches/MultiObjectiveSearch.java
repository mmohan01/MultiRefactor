...
public class MultiObjectiveSearch {
    ...
    private int initialRefactoringRange = 1;
    private String resultsPath = "./results/MOGeneticAlgorithm/";

    public MultiObjectiveSearch(Configuration[] c, String[] sourceFiles, String outputPath)
     {
        this.c = c;
    ...
    public void run()
     {
        ...
        ArrayList<RefactoringSequence> population = fitness(initialize());
                ...
                for (int j = 0; j < 2; j++)
                 {
                    ...
                    do
                        randomS2 = (int)(Math.random() * population.size()); while(randomS2 == randomS1);
        ...
        Metrics m = new Metrics(population.get(0).getServiceConfiguration());
    ...
    private ArrayList<RefactoringSequence> initialize()
     {
        ArrayList<Refactoring> refactorings = this.c[0].getRefactorings();
    ...
    private boolean isDominant(RefactoringSequence s1, RefactoringSequence s2)
     {
        ...
        FitnessFunction ff = new FitnessFunction();
        Metrics m1 = new Metrics(s1.getServiceConfiguration());
        Metrics m2 = new Metrics(s2.getServiceConfiguration());
        float value1, value2;
            ...
            for (int i = 0; i < this.c.length; i++)
             {
                for (RefactoringSequence s: paretoFront)
                 {
                    m = new Metrics(s.getServiceConfiguration());
                    ...
                    for (int j = 1; j < paretoFront.size() - 1; j++)
                     {
                        ...
                        newCrowdingDistance += (paretoFront.get(j + 1).getFitness() - paretoFront.get(j - 1).getFitness()) /
                                               (paretoFront.get(paretoFront.size() - 1).getFitness() - paretoFront.get(0).getFitness());
