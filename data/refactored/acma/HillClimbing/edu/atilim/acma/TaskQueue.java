...
public class TaskQueue {
    ...
    public synchronized void push(ConcurrentTask task) {
        queue.add(task);
        save();
    }

    public synchronized ConcurrentTask pop() {
        ConcurrentTask task = queue.poll();
        save();
        return task;
    }

    public synchronized ConcurrentTask peek() {
        return queue.peek();
    }

    public synchronized void remove(ConcurrentTask task) {
        queue.remove(task);
        save();
    }
    ...
    public synchronized List<ConcurrentTask> asList() {
