...
//				}
				...
				for (Field af: m.getAccessedFields())
					...
					mv.visitFieldInsn(Opcodes.GETFIELD, "pkg/Bean", af.getName(), "I");
				
				for (Method cm : m.getCalledMethods())
					// The opcode and field type isn't read in.
					mv.visitMethodInsn(Opcodes.INVOKESTATIC, cm.getOwnerType().getName(), cm.getName(), "I");
				
				for (Type it : m.getInstantiatedTypes())
					// The opcode isn't read in.
					mv.visitTypeInsn(Opcodes.NEW, it.getName());
