...
//		String []function=RoleManager.Access.getInstance().getRoleNames();
	...
	public static void save(File pdffile, /*String name, String desc, String orga,*/
			PrjInfos prjInfos,
		GanttProject prj, GanttTree tree, GanttGraphicArea area, ResourceLoadGraphicArea rarea,
		GanttExportSettings bool, String xslfo)	{
		try {											
			...
			ganttXML.append("\t\tnameValue=\""+ ((prjInfos.getName().length()==0)?" ":correct(prjInfos.getName())) +"\"\n");
			ganttXML.append("\t\torganisation=\""+GanttProject.correctLabel(language.getText("organization"))+"\"\n");
			ganttXML.append("\t\torganisationValue=\""+ ((prjInfos.getOrganization().length()==0)?" ":correct(prjInfos.getOrganization())) +"\"\n");
			ganttXML.append("\t\twebLink=\""+GanttProject.correctLabel(language.getText("webLink"))+"\"\n");
			ganttXML.append("\t\twebLinkValue=\""+ ((prjInfos.getWebLink().length()==0)?" ":correct(prjInfos.getWebLink())) +"\"\n");
			...
			ganttXML.append("\t\t\t"+ ((prjInfos.getDescription().length()==0)?" ":correct(prjInfos.getDescription())) +"\n");
			...
			ganttXML.append("\t\tmilestone=\"Bilan\"\n");
